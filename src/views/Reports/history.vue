    ...
    <div class="History-M">
        ...
        <div class="infolist">
          <div class="panel panel-Q">
            <panelTitle :panelTitleInit="panelTitle.queryTitle">
                <el-button data-id="openSearchDialog-btn" type="primary" size="mini" @click='openSearchDialog'>{{$t('vicenter.common.set')}}</el-button>
            </panelTitle>
          ...
          <div class="panel panel-L">
            <panelTitle :panelTitleInit="panelTitle.listTitle">
               ...
               <el-button v-if="exportButton" data-id="historyExportExcel-btn" type="primary" size="mini" @click='historyExportExcel'>{{$t('vicenter.common.export', {msg: 'EXCEL'})}}</el-button>
               <el-button data-id="openListDialog-btn" type="primary" size="mini" @click='openListDialog'>{{$t('vicenter.common.set')}}</el-button>
            </panelTitle>
        ...
        <el-dialog :title="searchTransfer.title" :visible.sync="searchTransfer.flag" :show-close='falseFlag' :close-on-click-modal='falseFlag' :close-on-press-escape='falseFlag' class='searchTransfer'>
          <div class="transfer-box" v-loading="isSavingSearch" element-loading-background="rgba(255, 255, 255, 0.8)">
            <el-transfer data-id="searchTransfer" v-if="isOpenSearch" v-model="searchTransfer.transfer.model" :data="searchTransfer.transfer.data" :titles="searchTransfer.transfer.title" :button-texts="searchTransfer.transfer.button" target-order="push" :right-default-checked="transferDefaultS"></el-transfer>
          </div>
          <div slot="footer" class="dialog-footer">
            <el-button data-id="saveSearchTranfer-btn" type="primary" :disabled="searchTransfer.transfer.model.length === 0 || isSavingSearch" @click="saveSearchTranfer">{{this.$t('vicenter.common.save')}}</el-button>
            <el-button data-id="dontSaveSearchTranfer-btn" :disabled="isSavingSearch" @click="dontSaveSearchTranfer">{{this.$t('vicenter.common.back')}}</el-button>
          </div>
        </el-dialog>
        <el-dialog :title="listTransfer.title" :visible.sync="listTransfer.flag" :show-close='falseFlag' :close-on-click-modal='falseFlag' :close-on-press-escape='falseFlag' class='searchTransfer'>
          <div class="transfer-box" v-loading="isSavingList" element-loading-background="rgba(255, 255, 255, 0.8)">
            <el-transfer data-id="listTransfer" v-if="isOpenList" v-model="listTransfer.transfer.model" :data="listTransfer.transfer.data" :titles="listTransfer.transfer.title" :button-texts="listTransfer.transfer.button" target-order="push" :right-default-checked="transferDefaultL"></el-transfer>
          </div>
          <div slot="footer" class="dialog-footer">
            <el-button data-id="saveListTranfer-btn" type="primary" :disabled="listTransfer.transfer.model.length === 0 || isSavingList" @click="saveListTranfer">{{this.$t('vicenter.common.save')}}</el-button>
            <el-button data-id="dontSaveListTranfer-btn" :disabled="isSavingList" @click="dontSaveListTranfer">{{this.$t('vicenter.common.back')}}</el-button>
          </div>
        </el-dialog>
  </div>
</template>
  ...
  methods: {
    setI18n () {
      ...
      this.tableGroupRes.forEach((e) => {
        ...
        } else if (e.model === 'analystAccount') {
          e.label = this.$t('vicenter.inspection.common.analystID')
          ...
          if (data.flag) {
            ...
            this.$data.tableData.data.forEach(obj => {
              ...
              obj.operation = [
                {
                  func: this.getDetail,
                  label: `<span class='iconfont editTable' data-id='getDetail-btn'>&#xe610;</span>`,
