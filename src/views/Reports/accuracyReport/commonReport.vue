...
export default {
    ...
    return {
      ...
      searchObj: {
        'INSPECTOR': {
          show: 'analystIds',
          hiden: ['recheckorIds'],
          chartTitle: this.$t('vicenter.reports.common.imageCheckAccuracyStatistic'),
          personInfo: this.$t('vicenter.reports.common.imgAnalyst'),
          textInfo: [
            {
              label: this.$t('vicenter.reports.common.imageCheckSuspectQty'),
              model: 'taskNum',
              icon: '&#xe701;'
            },
            {
              label: this.$t('vicenter.reports.common.imageCheckCorrectQty'),
              model: 'nNum',
              icon: '&#xe7f9;'
            },
            {
              label: this.$t('vicenter.reports.common.imageCheckErrorQty'),
              model: 'yNum',
              icon: '&#xe6aa;'
            }
          ],
          chartInfo: [
            {
              name: this.$t('vicenter.reports.common.imageCheckCorrectQty'),
              model: 'nNum'
            },
            {
              name: this.$t('vicenter.reports.common.imageCheckErrorQty'),
              model: 'yNum'
            }
          ],
          tableInfo: [
            ...
            {
              label: this.$t('vicenter.reports.common.imageCheckCorrectRate'),
              model: 'nNumRate'
            },
            {
              label: this.$t('vicenter.reports.common.imageCheckErrorRate'),
              model: 'yNumRate'
            }
          ]
        },
        'RECHECK-INSPECTOR': {
          show: 'recheckorIds',
          hiden: ['analystIds'],
          chartTitle: this.$t('vicenter.reports.common.recheckAccuracyStatistic'),
          personInfo: this.$t('vicenter.reports.common.recheckor'),
          textInfo: [
            {
              label: this.$t('vicenter.reports.common.recheckSuspectQty'),
              model: 'taskNum',
              icon: '&#xe701;'
            },
            {
              label: this.$t('vicenter.reports.common.recheckCorrectQty'),
              model: 'nNum',
              icon: '&#xe7f9;'
            },
            {
              label: this.$t('vicenter.reports.common.recheckErrorQty'),
              model: 'yNum',
              icon: '&#xe6aa;'
            }
          ],
          chartInfo: [
            {
              name: this.$t('vicenter.reports.common.recheckCorrectQty'),
              model: 'nNum'
            },
            {
              name: this.$t('vicenter.reports.common.recheckErrorQty'),
              model: 'yNum'
            }
          ],
          tableInfo: [
            ...
            {
              label: this.$t('vicenter.reports.common.recheckCorrectRate'),
              model: 'nNumRate'
            },
            {
              label: this.$t('vicenter.reports.common.recheckErrorRate'),
              model: 'yNumRate'
            }
          ]
        }
  ...
  methods: {
    ...
    query () {
      ...
      params.accountStr = params.analystIds.join(',') || params.recheckorIds.join(',') || ''
      ...
      // 获取审图、复审正确量
      ajax.getCorrectData(params).then((res) => {
        if (res.data.flag) {
          nNum = {
            'type': 'nNum',
            'items': res.data.result.items
          }
          this.commonCode(nNum, yNum, consistentNum, inconsistentNum)
        } else {
        }
      }).catch(() => {})
      // 获取审图、复审错误量
      ajax.getErrorData(params).then((res) => {
        if (res.data.flag) {
          yNum = {
            'type': 'yNum',
            'items': res.data.result.items
    ...
    commonCode (nNum, yNum, consistentNum, inconsistentNum) {
      // 统计数据
      ...
      // 列表数据
      this.tableData.data = this.getTableData(this.chartStatisticData[0].data, this.chartStatisticData[0].yAxisData, consistentNum.items, inconsistentNum.items)
      if (this.isChart) {
        this.$nextTick(() => {
    ...
    getTableData (data, people, consistentNumItem, inconsistentNumItem) {
      if (people && people.length > 0) {
        ...
        tableData.forEach((item, tIndex) => {
          let consistentArr = consistentNumItem.length > 0 ? consistentNumItem.filter(c => c.account === item.account) : []
          let inconsistentArr = inconsistentNumItem.length > 0 ? inconsistentNumItem.filter(c => c.account === item.account) : []
          item.consistentNum = consistentArr.length > 0 ? consistentArr[consistentArr.length - 1].taskNum : 0
          item.inconsistentNum = inconsistentArr.length > 0 ? inconsistentArr[inconsistentArr.length - 1].taskNum : 0
    ...
    getAverageAndTotal (tableData) {
      ...
      tableData.forEach(item => {
        ...
        totalObj.consistentNum += item.consistentNum
        totalObj.inconsistentNum += item.inconsistentNum
      ...
      aveObj = {
        ...
        consistentNum: (totalObj.consistentNum / len).toFixed(2),
        inconsistentNum: (totalObj.inconsistentNum / len).toFixed(2)
      }
    ...
    updateLabel () {
      // 控制图表标题切换
      this.chartStatisticData.forEach(item => {
        item.title = this.searchObj[this.searchIpt[0].models].chartTitle
    ...
    handleExportExcel () {
      ...
      let params = {
        ...
        url: '/supervise/accuracyRep/exportExcel'
      }
