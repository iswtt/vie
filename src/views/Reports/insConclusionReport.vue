...
export default {
  ...
  data () {
    return {
      ...
      printTable: {
        allColsNum: 6,
        title: this.$t('vicenter.reports.title.inspectionConclusionDetail'),
      ...
      chartStatisticData: [
        {
          title: this.$t('vicenter.reports.common.region') + ' - ' + this.$t('vicenter.reports.common.insConcStatistics'),
          baseTitle: this.$t('vicenter.reports.common.insConcStatistics'),
          id: 'insConcStatistics',
          chartType: 'bar',
          width: '100%',
          height: '300px',
          colors: ['#97d87c', '#ffd96e', '#bcbcbc', '#f4857c'],
          yAxisData: [],
          yAxisIds: [],
          data: [
            {
        ...
        {
          title: this.$t('vicenter.reports.common.insConcRatioStatistics'),
          baseTitle: this.$t('vicenter.reports.common.insConcRatioStatistics'),
          multiple: true,
          width: '100%',
          height: '250px',
          multipleData: [
            {
              chartType: 'ring',
              id: 'imgCheckNoSuspectRate',
              width: '33%',
              colors: '#97d87c',
              data: {
                name: this.$t('vicenter.reports.common.imgCheckNoSuspectRate'),
                model: 'noSuspectRate',
                value: 0
              }
            },
            {
              chartType: 'ring',
              id: 'suspectSeizedRate',
              width: '33%',
              colors: '#ffd96e',
              data: {
                name: this.$t('vicenter.reports.common.suspectSeizedRate'),
                model: 'suspectSeizedRate',
                value: 0
              }
            },
            {
              ...
              colors: '#48a9ee',
              data: {
                name: this.$t('vicenter.reports.common.taskSeizedRate'),
                model: 'seizedRate',
                value: 0
              }
            }
  ...
  methods: {
    ...
    commonCode (noSuspectQtyResult, suspectQtyResult, noSeizedQtyResult, seizedQtyResult) {
      ...
      this.textStatisticData[0].value = this.textStatisticData[1].value + this.textStatisticData[2].value + this.textStatisticData[3].value + this.textStatisticData[4].value
      // 排序
      let sortArea = this.getSortArea([noSuspectQtyResult.items, suspectQtyResult.items, noSeizedQtyResult.items, seizedQtyResult.items])
      ...
      // 环状图数据
      let imgCheckQty = this.textStatisticData[1].value + this.textStatisticData[2].value
      let radioObj = {
        noSuspectRate: imgCheckQty && this.textStatisticData[1].value ? (this.textStatisticData[1].value / imgCheckQty).toFixed(2) : 0,
        suspectSeizedRate: this.textStatisticData[2].value && this.textStatisticData[4].value ? (this.textStatisticData[4].value / this.textStatisticData[2].value).toFixed(2) : 0,
        seizedRate: imgCheckQty && this.textStatisticData[4].value ? (this.textStatisticData[4].value / imgCheckQty).toFixed(2) : 0
      }
    ...
    getTableData (data, area) {
      if (area && area.length > 0) {
        ...
        tableData.forEach((item, tIndex) => {
          ...
          item.noSeizedQty = tmpArr[2][tmpArr[2].length - tIndex - 1]
          item.seizedQty = tmpArr[3][tmpArr[3].length - tIndex - 1]
        })
