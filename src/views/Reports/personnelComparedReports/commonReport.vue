          ...
          <div class="chartBox" v-bar>
            <div class="chartBox-content">
              <div id="export-container">
                ...
                <tableStatistic v-if="switchChartList.list" data-id="personnelComparedTable" :showTable="showTable" :tableStatisticData="tableStatisticData"></tableStatistic>
              </div>
            </div>
          </div>
        </div>
...
import tableStatistic from '../common/tableStatistic'
export default {
    ...
    return {
      ...
      compareSearchIpt: [
        ...
        },
        {
          ...
          option: [],
          hiden: false,
          change: this.changeComparedPerson,
          tip: this.$t('vicenter.reports.validator.imgAnalystSelect')
        },
        {
          getLabel: this.$t('vicenter.reports.common.recheckor'),
      ...
      formRule: {
        ...
        refName: 'personnelComparedQueryForm',
        rule: {
          analystIds: [
      ...
      compareFormRule: {
        ...
        refName: 'personnelComparedQueryFormCompare',
        rule: {
          analystIds: [
            { required: true, message: this.$t('vicenter.reports.validator.imgAnalystSelect'), trigger: 'blur' }
          ],
      ...
      dimensionSearchObj: {
        'INSPECTOR': {
          ...
          chartInfo: this.$t('vicenter.reports.common.imgAnalystCompare'),
        ...
        'RECHECK-INSPECTOR': {
          ...
          chartInfo: this.$t('vicenter.reports.common.recheckorCompare'),
        ...
        'PHYSICAL-INSPECTOR': {
          ...
          chartInfo: this.$t('vicenter.reports.common.physicalInspectorCompare'),
        ...
        'AUDITOR': {
          ...
          chartInfo: this.$t('vicenter.reports.common.auditorCompare'),
  ...
  methods: {
    routeChange () {
      switch (this.$route.path) {
        case '/Reports/personnel-analysis-compared': {
          this.searchIpt[0].models = 'INSPECTOR'
          this.searchIpt[0].defaultValue = 'INSPECTOR'
          this.formRule.model.type = 'INSPECTOR'
          this.compareSearchIpt[0].models = 'INSPECTOR'
          this.compareSearchIpt[0].defaultValue = 'INSPECTOR'
          this.compareFormRule.model.type = 'INSPECTOR'
          break
        }
        case '/Reports/personnel-recheck-compared': {
          ...
          this.compareSearchIpt[0].models = 'RECHECK-INSPECTOR'
          this.compareSearchIpt[0].defaultValue = 'RECHECK-INSPECTOR'
          this.compareFormRule.model.type = 'RECHECK-INSPECTOR'
          break
        }
        case '/Reports/personnel-manual-compared': {
          this.searchIpt[0].models = 'PHYSICAL-INSPECTOR'
          this.searchIpt[0].defaultValue = 'PHYSICAL-INSPECTOR'
          this.formRule.model.type = 'PHYSICAL-INSPECTOR'
          this.compareSearchIpt[0].models = 'PHYSICAL-INSPECTOR'
          this.compareSearchIpt[0].defaultValue = 'PHYSICAL-INSPECTOR'
          this.compareFormRule.model.type = 'PHYSICAL-INSPECTOR'
          break
        }
        case '/Reports/personnel-audit-compared': {
          this.searchIpt[0].models = 'AUDITOR'
          this.searchIpt[0].defaultValue = 'AUDITOR'
          this.formRule.model.type = 'AUDITOR'
          this.compareSearchIpt[0].models = 'AUDITOR'
          this.compareSearchIpt[0].defaultValue = 'AUDITOR'
          this.compareFormRule.model.type = 'AUDITOR'
        }
      }
    },
    getPrintReportInfo () {
            ...
            if (res.data.result && res.data.result.length > 0) {
              this.searchIpt.forEach((item, index) => {
                if (i === item.roleCode) {
                  ...
                  this.compareSearchIpt[index].option = item.option
    ...
    queryCompare () {
      ...
      paramsCompare.accountStr = paramsCompare.analystIds || paramsCompare.recheckorIds || paramsCompare.manualIds || paramsCompare.auditorIds || ''
      delete paramsCompare.analystIds
      delete paramsCompare.recheckorIds
      delete paramsCompare.manualIds
      delete paramsCompare.auditorIds
      ...
      for (var i in this.dimensionSearchObj) {
        this.dimensionSearchObj[i].barLegend.forEach(item => {
          item.seriesData = []
      ...
      this.tableGroupDefault.forEach(item => {
        if (item.dimensionType === this.formRule.model.type || item.dimensionType === 'common') {
      ...
      // 打印信息处理
      this.printTable.infoData[1].data[0].value = this.dimensionSearchObj[params.type].dimensionInfo
      this.printTable.infoData[1].data[1].value = params.accountStr
      this.printTable.infoData[1].data[3].value = this.typeSearchObj[params.dateType].typeInfo
      if (params.dayTime) {
        this.printTable.infoData[1].data[5].value = this.$options.filters['formatDate'](params.dayTime, this.typeSearchObj[params.dateType].format)
      } else {
        this.printTable.infoData[1].data[5].value = this.$options.filters['formatDate'](params.startTime, this.typeSearchObj[params.dateType].format) + ' ~ ' + this.$options.filters['formatDate'](params.endTime, this.typeSearchObj[params.dateType].format)
      }
      this.printTable.infoData[2].data[0].value = this.dimensionSearchObj[paramsCompare.type].dimensionInfo
      this.printTable.infoData[2].data[1].value = paramsCompare.accountStr
      this.printTable.infoData[2].data[3].value = this.typeSearchObj[paramsCompare.dateType].typeInfo
      if (paramsCompare.dayTime) {
        this.printTable.infoData[2].data[5].value = this.$options.filters['formatDate'](paramsCompare.dayTime, this.typeSearchObj[paramsCompare.dateType].format)
      } else {
        this.printTable.infoData[2].data[5].value = this.$options.filters['formatDate'](paramsCompare.startTime, this.typeSearchObj[paramsCompare.dateType].format) + ' ~ ' + this.$options.filters['formatDate'](paramsCompare.endTime, this.typeSearchObj[paramsCompare.dateType].format)
      }
      this.exportExcelParams = [params, paramsCompare]
      console.log(params)
      console.log(paramsCompare)
      ajax.getUserComparedData(params).then((res) => {
        if (res.data.flag) {
          let result = res.data.result
          this.commonCode(result, params.type, params.accountStr, params.dateType, params.dayTime, 0)
        }
      }).catch(() => {})
      ajax.getUserComparedData(paramsCompare).then((res) => {
        if (res.data.flag) {
          let result = res.data.result
          this.commonCode(result, paramsCompare.type, paramsCompare.accountStr, paramsCompare.dateType, paramsCompare.dayTime, 1)
        }
      }).catch(() => {})
    },
    commonCode (result, type, accountStr, dateType, dayTime, dataType) {
      // 更新图表标题内容
      this.chartTitle.text = this.dimensionSearchObj[this.formRule.model.type].chartInfo
      // 加载图表数据
      let date = ''
      if (dateType === 'YEAR') {
      ...
      this.xAxisData[dataType] = `${accountStr} ${date}`
      this.barChartStatisticData[0].xAxisData = this.xAxisData
      for (var i in this.dimensionSearchObj) {
        if (type === i) {
          if (type === 'INSPECTOR' || type === 'RECHECK-INSPECTOR') {
            this.dimensionSearchObj[i].barLegend[0].seriesData[dataType] = result.noSuspectNum || 0
            this.dimensionSearchObj[i].barLegend[1].seriesData[dataType] = result.suspectNum || 0
          } else if (type === 'PHYSICAL-INSPECTOR') {
            this.dimensionSearchObj[i].barLegend[0].seriesData[dataType] = result.noSeizedNum || 0
            this.dimensionSearchObj[i].barLegend[1].seriesData[dataType] = result.seizedNum || 0
          } else if (type === 'AUDITOR') {
            this.dimensionSearchObj[i].barLegend[0].seriesData[dataType] = result.recheckPassNum || 0
            this.dimensionSearchObj[i].barLegend[1].seriesData[dataType] = result.recheckNotPassNum || 0
          }
      ...
      this.tableAllData[dataType].user = accountStr
      // 加载列表数据
    ...
    resetFormCompare () {
      ...
      this.searchIpt[5].models = this.compareSearchIpt[5].models
    ...
    getTableData (type) {
      ...
      let user = this.tableAllData[0].user
      let compareUser = this.tableAllData[1].user
    ...
    changeCompareTypeCompare () {
      ...
      this.compareSearchIpt.forEach(item => {
        for (let i in this.typeSearchObj) {
          if (this.compareSearchIpt[5].models === i) {
            if (item.ruleProp === this.typeSearchObj[i].show) {
              item.hiden = false
              item.models = item.defaultValue
    ...
    handleExportExcel () {
      ...
      let params = {
        ...
        sysMenu: '',
        allDataNum: 2,
        url: '/supervise/taskCompareRep/exportUsrTaskCompare'
      }
    ...
    changeComparedPerson () {
      for (let i in this.dimensionSearchObj) {
        if (this.searchIpt[0].models === i) {
          this.$changeSelectValidateField('compareSearchFormRef', this, this.dimensionSearchObj[i].show)
        }
      }
    },
    resetForm () {},
