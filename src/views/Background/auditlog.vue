<template>
  <div class='background-auditlog-bar' v-bar>
    <div class="background-auditlog-M">
      ...
      <div class="modal-dialog">
        <el-dialog :title="$t('vicenter.Background.auditLog.log.logInfo')" :visible.sync="detailDia.flag" class='detailLog' :show-close='falseflag' :close-on-click-modal='falseflag' :close-on-press-escape='falseflag'>
          ...
          <div slot="footer" class="dialog-footer">
            <el-button @click="detailDia.flag = false" size="mini">{{$t('vicenter.common.close')}}</el-button>
          </div>
        </el-dialog>
      </div>
    </div>
...
<script>
import ajax from '@/api/background/log.js' // 接口文件
  ...
  data () {
    return {
      ...
      formRule: { // 查询表单规则
        ...
        rule: {
          clientIp: [
            { pattern: /^[0-9.]*$/, message: this.$t('vicenter.common.verifyRules.onlyDP'), trigger: 'change' }
          ],
          operateAccount: [
      ...
      formIpt: [ // 查询表单初始化
        ...
        {
          getLabel: this.$t('vicenter.Background.auditLog.log.appCode'),
          type: 'select',
          placeHolder: this.$t('vicenter.common.tips.pleaseSelect', {msg: this.$t('vicenter.Background.auditLog.log.appCode')}),
          clearable: true,
          models: '',
          option: [
            {
              value: 'INS',
              label: this.$t('vicenter.Background.auditLog.appCode.INS')
            },
            {
              value: 'DMS',
              label: this.$t('vicenter.Background.auditLog.appCode.DMS')
            },
            {
              value: 'MGMT',
              label: this.$t('vicenter.Background.auditLog.appCode.MGMT')
            },
            {
              value: 'TIP',
              label: this.$t('vicenter.Background.auditLog.appCode.TIP')
            },
            {
              value: 'TIL',
              label: this.$t('vicenter.Background.auditLog.appCode.TIL')
            },
            {
              value: 'REPORT',
              label: this.$t('vicenter.Background.auditLog.appCode.REPORT')
            }
          ],
          ruleProp: 'appCode'
        }
      ...
      tableGroup: [ // 表格数据列
        ...
        {
          label: this.$t('vicenter.Background.auditLog.log.appCode'),
          type: 'text',
          model: 'appCode'
        }
        // {
  ...
  methods: {
      ...
      ajax.AduitLog(val).then((res) => {
        if (res.data.flag) {
          ...
          this.tableData.data.forEach(e => {
            ...
            if (e.appCode) {
              e.appCode = this.$t('vicenter.Background.auditLog.appCode.' + e.appCode)
            }
          })
    ...
    SetpageQuery (page) { // 查询设置页码
      ...
      var value = {
        ...
        appCode: this.formRule.model.appCode
      }
...
.background-auditlog-bar {
  @extend .extend-bar;
  .background-auditlog-M {
    padding:$margin;
    @include box-sizing;
    .form-bar {
      width: $all;
