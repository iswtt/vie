<template>
  <div class='studentReview-bar' v-bar>
    <div class="studentReview-M">
      ...
      <div class="infolist">
        ...
        <div class="panel panel-L">
          ...
          <div class="form-bar" v-bar>
            <ComTable ref="studentReviewTable" :tableData="tableData" :tableGroup="tableGroup" ></ComTable>
          </div>
        </div>
      </div>
    </div>
  ...
  methods: {
    setI18n () {
      ...
      this.searchIpt.forEach((e) => {
        if (e.ruleProp === 'trainingClassification') {
        ...
        } else if (e.ruleProp === 'trainingName') {
    ...
    handleCreate () {
      ...
      window.open(`${this.baseUrl}/report/exportReviewCount?${str}language=${lang}`)
    },
    resetForm () {
      this.formRule.model = {
        account: '',
    ...
    getList (params) {
      req.reviewCount({ ...params, page: this.tableData.page.CurrentPage, size: this.tableData.page.pagesize })
        .then(res => {
          let tableData = res.data.result
          tableData.content.forEach(item => {
            item.correctRate = (item.correctRate + '%')
