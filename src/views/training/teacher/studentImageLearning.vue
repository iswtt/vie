<template>
  <div class='studentImageLearning-bar' v-bar>
    <div class="studentImageLearning-M">
      ...
      <div class="infolist">
        ...
        <div class="panel panel-L">
          ...
          <div class="form-bar" v-bar>
            <ComTable ref="studentImageLearningTable" :tableData="tableData" :tableGroup="tableGroup" ></ComTable>
          </div>
        </div>
      </div>
    </div>
  ...
  methods: {
    setI18n () {
      ...
      this.tableGroup.forEach((e) => {
        ...
        } else if (e.model === 'trainingClassification') {
          e.label = this.$t('vicenter.training.common.trainingClassification')
        } else if (e.model === 'trainingName') {
    ...
    handleCreate () {
      ...
      window.open(`${this.baseUrl}/report/exportLearnCount?${str}language=${lang}`)
    },
    resetForm () {
      this.formRule.model = {
        account: '',
    ...
    getList (params) {
      req.learnCount({ ...params, page: this.tableData.page.CurrentPage, size: this.tableData.page.pagesize })
        .then(res => {
...
.studentImageLearning-bar {
  @extend .extend-bar;
  .studentImageLearning-M {
    padding:$margin;
    @include box-sizing;
    .form-bar {
      width: $all;
