  ...
  <div class='image-Edit-Tip-bar'>
    <div class="image-Edit-Tip-M">
      ...
      <div class="operation-group">
        <el-button type="white" size="mini" @click='goBack'>{{$t('vicenter.common.back')}}</el-button>
        <el-button type="primary" size="mini" @click='handleSave'>{{$t('vicenter.common.save')}}</el-button>
      </div>
        ...
        <div class="panel panel-up">
          ...
          <div class="containerBox">
            <div class="dataBox">
              ...
              <div class="fileNames">
                <fileNames @deleteThen = 'deleteImgFile' :data="saveImg.files" :source="fileSource"></fileNames>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    insDbclick (row, $event) { // 监管选图
      let {id, scanSerialNumber, lastScanImage, scanImagePath} = row
    ...
    imgLibDbclick (row, $event) { // 典图选图
      ...
      let {scanSerialNumber, lastImage, fileList} = row
    ...
    getOpenLocalFileName () {
      if (viprobeReturn.openLocalResult.flag === null) {
        if (viprobeUtilReturn.reconnectToViProbeFailed === true) {
          ...
          setTimeout(this.getOpenLocalFileName, this.viprobeData.timeout)
        }
    ...
    openViprobe (params) {
      ...
      insCommonAjax.getFullSignPath(data).then((res) => {
        ...
        viProbe.openImageAndAnalysisInfoInViProbe(scanSerialNumber, scanImagePath, files, markFilePath, stepFilePath, isUff)
      })
    },
    closeViprobe () {
