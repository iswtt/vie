<template>
  <div class='suspectImageMsg-bar' v-bar>
    <div class="suspectImageMsg-M">
      ...
      <Dialog :modalData="modalData" class="messageDialog">
        ...
        <div v-if="failReason.length>0">
          ...
          <ul class="messageDialog-ul" data-id="messageDialog-ul">
            <li v-for="(tmp, index) in failReason" :key='index'>{{tmp.title}}：{{$t(tmp.note)}}..............</li>
          </ul>
        </div>
...
export default {
    ...
    return {
      ...
      tableGroup: [ // 表格数据列
        ...
        },
        {
          type: 'text',
          // model: 'hsCodes'
          model: window.location.href.indexOf('suspectRegulatory') !== -1 ? 'hsCodeCvs' : 'hsCodes'
        }
      ],
      formRule: { // 查询表单规则
        model: {
  ...
  created () {
    this.tempIndex = 1
    this.setI18n()
    // let url = 'https://10.44.162.234/nuctechfile/TIL/intelImage/2019/1206/254010D01201806130032_icon.jpg?a=81369382&b=55b618e887ed472db0cb146ba37a405d&c=a36622b9f8a29dfe7d47af296711e546'
  },
  computed: {
    comma () {
  ...
  methods: {
    ...
    setI18n () {
      ...
      if (this.$route.path.indexOf('suspectCode') !== -1) {
        this.searchIpt.push(
          {
            type: 'text',
            models: '',
    ...
    query () {
      ...
      if (this.$route.path.indexOf('suspectCode') !== -1) {
        // 典图查询调价
        params.imageType = 'ILLEGAL'
      } else {
        // 监管的查询条件
        params.concImgcheck = 'CONC001' // 嫌疑图
      }
      ...
      ajax.getimglist(params).then((res) => {
        if (res.data.flag) {
          ...
          this.tableData.data.forEach(item => {
            ...
            let imgsUrl = oldImgsUrl.substring(0, oldImgsUrl.indexOf('?'))
            // let reg = /^((https):\/\/)(([0-9]{1,3}\.){3}[0-9]{1,3})*(\/)/
            // imgsUrl = imgsUrl.replace(reg, this.saveImg.server_path)
            let url = imgsUrl.substring(0, imgsUrl.lastIndexOf('/') + 1)
            if (item.lastImage) {
            ...
            // 列表显示的缩略图
            // item.thumb = url + 'grab_img_suspects.jpg'
            item.thumb = oldImgsUrl
          })
    ...
    // 返回
    back () {
      this.$router.push({path: '/Training/manager/suspectImage'})
    },
    // 导入
    handleCreate () {
      ...
      if (this.$route.path.indexOf('suspectCode') !== -1) {
        ...
        // 监管导入
        ajax.getImgDetail({
          ids: this.selRowId.map(e => {
            return e.testImgApiId
          }).join(',')
        }).then(res => {
          /*
               * 通过列表id 获取详情中图片地址
          ...
          this.selRowId.forEach(item => {
            item.markUrl = ''
            let url = ''
            item.testImgXyType = rep[item.testImgApiId].seizeType
            let a = rep[item.testImgApiId].taskAttachementObjectlist.filter(e => {
              ...
              url = oldImgUrl.substring(0, oldImgUrl.lastIndexOf('/') + 1)
              item.markUrl || (item.markUrl = url + 'suspects_json.mark')
              return {
                imgsName: i.fileName,
                imgsUrl: url + i.fileName,
                imgsDocType: i.docType,
                imgMd5: i.imgMd5,
                imgsEyes: i.viewIndex,
                enegryType: i.energyType,
                suspectThumbnail: url + 'grab_img_suspects.jpg'
              }
            })
            a.forEach(e => {
              item.imgsJson.push({
                ...
                originalThumbnail: url + item.lastScanImage + '.jpg',
                suspectThumbnail: url + 'grab_img_suspects.jpg'
              })
            })
            // item.imgsJson = {
            ...
            //   suspectThumbnail: a.map(e => { return e.suspectThumbnail }).join(','),
      ...
      this.$data.selRowId = data.map(e => {
        ...
        if (this.$route.path.indexOf('suspectCode') !== -1) {
          ...
          imgsJson = e.fileList.filter(e => {
            ...
            let oldImgsUrl = e.lastImage ? e.lastImage : e.lastScanImage
            let url = oldImgsUrl.substring(0, oldImgsUrl.lastIndexOf('/') + 1)
            markUrl || (markUrl = url + 'suspects_json.mark')
            return {
              imgsName: f.fileName,
              imgsUrl: url + f.fileName,
              imgsDocType: f.docType,
              imgMd5: f.imgMd5,
              imgsEyes: f.viewIndex,
              enegryType: f.energyType,
              // originalThumbnail: url + e.scanSerialNumber + '.jpg',
        ...
        return {
          ...
          lastScanImage: e.scanSerialNumber,
          testImgXyType: e.seizedType, // TODO 待接口修改后再调整
          testImgApiId: this.$route.path.indexOf('suspectCode') !== -1 ? e.scanSerialNumber : e.id,
          testImgName: this.$route.path.indexOf('suspectCode') !== -1 ? e.imageName : e.scanSerialNumber,
          testImgHashCodes: this.$route.path.indexOf('suspectCode') !== -1 ? e.hsCodes : e.hsCodeCvs,
          hsIds: this.$route.path.indexOf('suspectCode') !== -1 ? e.hsIds : e.hsCodeCvs,
          testImgHashNames: this.$route.path.indexOf('suspectCode') !== -1 ? e.hsNames : e.hsNameCvs,
          testImgClass: 1,
          promptMsg: e.scanSerialNumber,
          testImgCreateSource: this.$route.path.indexOf('suspectCode') !== -1 ? 1 : 2,
          // markUrl: url + 'suspects_json.mark'
          markUrl: markUrl
        }
      })
    ...
    SetpageQuery (page) { // 查询设置页
      ...
      if (this.$route.path.indexOf('suspectCode') !== -1) {
        // 典图查询调价
        value.imageType = 'ILLEGAL'
      } else {
        // 监管的查询条件
        value.concImgcheck = 'CONC001' // 嫌疑图
      }
