<template>
  <div class='image-query-bar' v-bar>
    <div class="image-query-M">
        ...
        <div class="panel panel-L">
          <panelTitle :panelTitleInit="panelTitle.listTitle">
            ...
            <div class="tabViewBtn">
              <el-button type="text" @click='tabView' :title="$t('vicenter.ImageLibrary.common.List')" :disabled="isList" data-id="list-btn"><span class='iconfont editTable btnClass viewClass' :class="{notActive: isList}">&#xe661;</span></el-button>
              <el-button type="text" @click='tabView' :title="$t('vicenter.ImageLibrary.common.Thumbnail')" :disabled="!isList" data-id="thumbnail-btn"><span class='iconfont editTable btnClass viewClass' :class="{notActive: !isList}">&#xe65f;</span></el-button>
            </div>
          </panelTitle>
          <div class="form-bar" v-bar>
            <ComTable ref="imageTable" :tableData="tableData" :tableGroup="tableGroup" ></ComTable>
          </div>
        </div>
      </div>
...
export default {
          ...
          this.tableData.data.forEach(item => {
            ...
            this.operation.forEach((e, index) => {
              if (item.imageStatus === 'TEMPORARY') {
                ...
                // 已拒绝
                operateArr = [0, 3]
              } else if (item.imageStatus === 'PUBLISHED') {
                // 已发布
                operateArr = [0]
              }
  ...
  beforeRouteLeave (to, from, next) {
    if (to.path !== '/language' && to.path !== '/skin' && to.path !== '/ImageLibrary/imageQuery/imageDetails') {
