<template>
  <div class='Manage-Image-bar' v-bar>
    <div class="Manage-Image-M">
...
import ajax from '@/api/inspection/Manage-Image.js'
...
export default {
    ...
    return {
      ...
      inputItemRes: [
        ...
        },
        {
          ...
          option: [
            // {
            //   value: 'WAITING_IMAGE_CHECK',
            //   label: this.$t('vicenter.DIC.T_TASK_STATUS.C_WAITING_IMAGE_CHECK')
            // },
            // {
            //   value: 'IMAGE_CHECK_LOCKED',
            //   label: this.$t('vicenter.DIC.T_TASK_STATUS.C_IMAGE_CHECK_LOCKED')
            // }
          ]
        },
        {
          // getLabel: this.$t('vicenter.inspection.common.conclusionType'),
          type: 'select',
          // placeHolder: this.$t('vicenter.common.pleaseSelcet'),
          models: '',
          ruleProp: 'taskLink',
          ...
          option: [
            // {
            //   value: 'IMAGE_CHECK',
            //   label: this.$t('vicenter.DIC.T_TIMELINE_NODE.C_IMAGE_CHECK')
            // },
            // {
            //   value: 'SECOND_IMAGE_CHECK',
            //   label: this.$t('vicenter.DIC.T_TIMELINE_NODE.C_SECOND_IMAGE_CHECK')
            // }
          ]
        },
        {
      ...
      const: {
        taskLink: {
          inspection: 'IMAGE_CHECK', // 初审
          recheck: 'SECOND_IMAGE_CHECK' // 复审
        },
        roleCode: {
          inspection: 'INSPECTOR ', // 审图员
          recheck: 'RECHECK-INSPECTOR' // 复审员
        }
      },
      inputItemOrigin: [],
      inputItem: [],
  ...
  methods: {
    setI18n () {
      ...
      this.inputItemRes.forEach((e) => {
        ...
        } else if (e.ruleProp === 'taskLink') {
          e.getLabel = this.$t('vicenter.inspection.common.conclusionType')
          // e.placeHolder = this.$t('vicenter.common.pleaseSelcet')
          e.placeHolder = this.$t('vicenter.common.tips.pleaseSelect', {msg: this.$t('vicenter.inspection.common.conclusionType')})
          e.option = [
            {
              value: 'IMAGE_CHECK',
              label: this.$t('vicenter.DIC.T_TIMELINE_NODE.C_IMAGE_CHECK')
      ...
      this.tableGroupRes.forEach((e) => {
        ...
        } else if (e.model === 'taskLinkVal') {
          e.label = this.$t('vicenter.inspection.common.conclusionType')
        ...
        if (res.status === 200) {
          ...
          if (data.flag) {
            ...
            this.$data.tableData.data.forEach(obj => {
              ...
              obj.taskLinkVal = this.$t(`vicenter.DIC.T_TIMELINE_NODE.C_${obj.taskLink}`)
    ...
    getAssignTaskData (scope) { // 获取分配任务数据
      let {id, taskLink} = scope.row
      let roleCode = ''
      if (taskLink === this.const.taskLink.inspection) {
        roleCode = this.const.roleCode.inspection
      } else if (taskLink === this.const.taskLink.recheck) {
        roleCode = this.const.roleCode.recheck
      }
      commonAjax.listPortAndPersionByRoleCode({roleCode: roleCode}).then(res => {
        if (res.data.flag) {
          ...
          this.assignTaskTable.tableData.data = result
          this.assginTaskData.taskLink = taskLink
    ...
    assignTask () { // 分配任务操作
      ...
      data.taskLink = this.assginTaskData.taskLink
    ...
    exportExcel () {
      ...
      let params = {
        ...
        url: '/inspection/taskmgmt/exportExcel'
      }
  ...
  beforeRouteLeave (to, from, next) {
    if (to.path !== '/language' && to.path !== '/skin' && to.path !== '/Inspection/Manage-Image-detail') {
