...
export default {
  ...
  methods: {
    ...
    closeTipResultModal () {
      ...
      this.formRule.rule = this.noSuspectedRule
      this.formRule.model = {
    ...
    getDetail () {
      if (this.id === -1) {
        ...
        ajax.getDetail({id: this.id}).then(res => {
          if (res.data.flag) {
            let { taskObject, taskDecObjects, taskTargetObjects, taskAttachementObjects, tipExamObject, imgCheckObjects } = res.data.result
            ...
            if (imgCheckObjects && imgCheckObjects.length > 0) {
              imgCheckObjects.forEach(item => {
                if (item.taskLink.indexOf('C_IMAGE_CHECK') > -1) {
    ...
    judgeOpen () {
      if (window.viprobeReturn.openImgSuccess === null) {
        ...
        if (!window.viprobeReturn.openImgSuccess) {
          ...
          if (!this.tipExamId && this.AIResultData.jsonData && this.$isJsonString(this.AIResultData.jsonData)) {
