...
export default {
  ...
  methods: {
    setI18n () {
      this.orgAjaxTree()
      this.deviceAjaxTree()
      this.$getDataByTypecode(['IMG_CHECK_CONC'], ['concImgcheck'], this.inputItem)
      // 查询条件输入框校验
      ...
      this.inputItem.forEach((e) => {
        if (e.ruleProp === 'siteId') {
          ...
          e.getLabel = this.$t('vicenter.inspection.common.imgConclusion')
          // e.placeHolder = this.$t('vicenter.common.pleaseSelcet')
          e.placeHolder = this.$t('vicenter.common.tips.pleaseSelect', {msg: this.$t('vicenter.inspection.common.imgConclusion')})
        }
      })
      // 查询列
      this.tableGroup.forEach((e) => {
        if (e.model === 'lastScanImage') {
          ...
          e.label = this.$t('vicenter.inspection.common.imgConclusion')
            ...
            obj.operation = [
              {
                ...
                title: this.$t('vicenter.inspection.btn.recheck'),
                show: true
              }
            ]
            obj.lastScanImage = {
  ...
  beforeRouteLeave (to, from, next) {
    if (to.path !== '/language' && to.path !== '/skin' && to.path !== '/Inspection/Recheck-detail') {
