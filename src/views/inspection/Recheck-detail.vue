<template>
  <div class='Recheck-detail-bar' v-bar>
    <div class='Recheck-detail-M'>
        ...
        <div class="up-down-infolist">
          <div class="up-down-panel">
            <panelTitle :panelTitleInit="panelTitle.queryTitle">
              ...
              <el-button v-if="imageLibraryEnable && showAIResult" class="AIResult" type="primary" size="mini" data-id='showAIResult-btn' @click='showAIResultModal'>
                {{$t('vicenter.common.AIResult')}}
                <span v-if="AIResultData.jsonData && $isJsonString(AIResultData.jsonData)" class="mark" :class="{mark_red:AIResultData.conclusion!=='NO_SUSPECT',mark_green:AIResultData.conclusion==='NO_SUSPECT'}"></span>
              </el-button>
              <el-button class="findImg" type="primary" size="mini" data-id="findImage-btn" @click='showFindImgDialog'>{{$t('vicenter.common.FindImage')}}</el-button>
              <el-button type="primary" size="mini" data-id="submit-btn" @click="judgeSubmit()">{{$t('vicenter.common.submit')}}</el-button>
              <el-button size="mini" data-id="quit-btn" @click="quitTask()">{{$t('vicenter.common.quit')}}</el-button>
            </panelTitle>
            <div class="image-info-box">
              ...
              <el-col :span="24" class="flex">
                ...
                <div class="border physicalBox flex">
                  <panelTitle :panelTitleInit="title.RecheckConclutionTitle"></panelTitle>
...
import ajax from '@/api/inspection/Recheck.js'
...
export default {
  ...
  data () {
    return {
      ...
      panelTitle: { // title组件数据
        queryTitle: {
          title: this.$t('vicenter.common.menus.inspectionChild.recheckDetail')
        }
      },
      title: {
        ImageAnalysisTitle: {
        ...
        RecheckConclutionTitle: {
          title: this.$t('vicenter.inspection.common.recheckConclution')
        },
      ...
      inputItem: [
        {
          getLabel: this.$t('vicenter.inspection.common.recheckConclution'),
          type: 'radio',
          placeHolder: '',
          models: '',
          ruleProp: 'conclusion',
  ...
  methods: {
      ...
      if (this.id === -1) {
        ...
        ajax.getDetail({id: this.id}).then(res => {
          if (res.data.flag) {
            ...
            this.firstImgCheck = imgCheckObjects
            // 扫描图片
    ...
    judgeImgCheck () {
      ...
      let firstImgCheck = ''
      if (this.firstImgCheck && this.firstImgCheck.length > 0) {
        firstImgCheck = this.firstImgCheck[0].conclusion
      }
      if (firstImgCheck.indexOf(this.viprobeData.suspected) > -1 && conclusion === this.viprobeData.noSuspected) {
        flag = false
      }
      return flag
    },
    ...
    judgeSubmit () {
      ...
      // 初审有嫌疑，复审不能无嫌疑
      if (!this.judgeImgCheck()) {
        this.$NotifPrompt(this.$t('vicenter.inspection.common.reCheckRuleInfo'))
        return false
      }
      ...
      this.$refs['imageCheck'].$refs[this.formRule.refName].validate((valid) => {
        ...
        if (valid) {
          ...
          this.$ConfirmBox(this.$t('vicenter.inspection.common.submitTask'), () => {
            ...
            // 第2、3、4、步骤的操作如下所示-----------------------------------------
            // 清除所有要用的变量的值，设置为初始值
            console.debug('start getMarkAsJson')
            viProbe.getNewMarkAsJson(this.scanningImgName)
    ...
    markValidator () {
      ...
      console.log('window' + viprobeReturn.getMarkJsonSuccess)
      if (viprobeReturn.getMarkJsonSuccess === null) {
      ...
      } else if (viprobeReturn.getMarkJsonSuccess === true) {
        viprobeReturn.getMarkJsonSuccess = null
        if (conclusion === this.viprobeData.suspected && viprobeReturn.markJson !== '' && viprobeReturn.markJson.length !== 0) {
          console.debug('end validate Mark, start upload viprobe info')
          let params = {
            paths: `${path}suspects_json.mark,${path}history.steps,${path}grab_img_suspects.jpg`
          }
          this.markString = JSON.stringify(viprobeReturn.markJson)
          let markSign = ''
          let stepSign = ''
          let markJpgSign = ''
          // 获取嫌疑标记和历史记录的签名
            ...
            if (res.data.flag) {
              res.data.result.forEach(e => {
                if (e.indexOf('suspects_json.mark') !== -1) {
                  markSign = e.substring(e.indexOf('?') + 1)
                } else if (e.indexOf('history.steps') !== -1) {
                  stepSign = e.substring(e.indexOf('?') + 1)
                } else if (e.indexOf('grab_img_suspects.jpg') !== -1) {
                  markJpgSign = e.substring(e.indexOf('?') + 1)
                }
              })
            }
        ...
        } else if (conclusion === this.viprobeData.noSuspected && (viprobeReturn.markJson === '' || viprobeReturn.markJson.length === 0)) {
          console.debug('end validate Mark, start upload viprobe info')
          let params = {
            paths: `${path}history.steps`
          }
          let stepSign = ''
          // 获取嫌疑标记和历史记录的签名
          insCommonAjax.getFullSignPath(params).then((res) => {
      ...
      } else if (viprobeReturn.getMarkJsonSuccess === false) {
        viprobeReturn.getMarkJsonSuccess = null
    ...
    submitTask () {
      ...
      ajax.submitTask(data).then(res => {
        if (res.data.flag) {
          ...
          this.$router.push({ path: '/Inspection/Recheck' })
        } else {
          this.setButtonEnable()
        }
      }).catch(() => {
    ...
    openViprobe () {
      ...
      insCommonAjax.getFullSignPath(params).then((res) => {
        ...
        viProbe.openImageAndAnalysisInfoInViProbe(name, path, imageFiles, markFilePath, stepFilePath, '#FFFF00', isUff)
      ...
      if (window.viprobeReturn.openImgSuccess === null) {
        ...
        if (!window.viprobeReturn.openImgSuccess) {
          if (window.viprobeReturn.errorCode === 4105) { // 图像被意外篡改
            // this.tamperedStatus可能有三个值：
            // 0：表示现场未篡改，此时tamperedPosition为中心篡改，1
            // 1：表示现场已篡改，此时tamperedPosition为现场篡改，2
            // 2：表示现场未校验，此时tamperedPosition为未校验，4
            let params = {
              taskId: this.id,
              tamperedPosition: ''
            }
            if (this.tamperedStatus === 0) {
              params.tamperedPosition = '1'
            } else if (this.tamperedStatus === 1) {
              params.tamperedPosition = '2'
            } else if (this.tamperedStatus === 2) {
              params.tamperedPosition = '4'
            }
            tamperRecordAjax.addRecord(params).then(res => {
            })
          }
  ...
  components: {breadCrumb, panelTitle, ComTable, ComForm, Cswiper, doubleSwipe, insTitle, asycuda, detailList, Dialog, TabList, AIResult, DocInfo}
}
</script>
