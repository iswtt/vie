<template>
  <div class='manage-image-detail-bar' v-bar>
    <div class='manage-image-detail-M'>
...
export default {
  ...
  data () {
    return {
      ...
      panelTitle: { // title组件数据
        queryTitle: {
          title: this.$t('vicenter.inspection.common.inspectionTask')
        }
      },
      title: {
    ...
    getDetail () {
      if (this.id === -1) {
        ...
        ajax.getDetail({id: this.id}).then(res => {
          if (res.data.flag) {
            let { taskObject, taskTimeLineObjects, taskDecObjects, taskTargetObjects, taskAttachementObjects, imgCheckObjects } = res.data.result
      ...
      this.$ConfirmBox(this.$t('vicenter.inspection.common.unlockTask'), () => {
        ajax.unlocktasks({ids: this.id}).then(res => {
          if (res.data.flag) {
            ...
            this.$router.push({ path: '/Inspection/Manage-Image' })
          }
        }).catch(() => {})
      })
    },
  ...
  components: {breadCrumb, panelTitle, ComTable, ComForm, Cswiper, doubleSwipe, timeSteps, asycuda, detailList, Dialog, TabList, DocInfo}
}
</script>

<style lang="scss">
@import "@/styles/index.scss";
.manage-image-detail-bar {
  @extend .extend-bar;
  .manage-image-detail-M{
    .steps{
      width: $all;
